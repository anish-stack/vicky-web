...
export default function Contact() {
  ...
  const methods = [
    { icon: Phone, title: "Call Now", value: `+91 ${phone}`, link: `tel:+91${phone}` },
  ...
  const inputCls =
    "w-full px-4 py-3 rounded-xl border-2 border-slate-200 bg-[#fffdf5] text-slate-900 text-sm font-bold placeholder-slate-400 outline-none focus:border-green-700";

  return (
    <section id="contact" className="bg-green-700 relative">
      ...
      <div className="py-12 md:py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 md:mb-14">
          ...
          <h2 className="mt-5 text-3xl md:text-5xl font-black text-white">
            Give Us a Shout, <span className="text-amber-300">We'll Be There</span> 🚕
          </h2>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
              ...
              return (
                <Wrapper
                  ...
                  {...(m.link
                    ...
                    : {})}
                  className={`rounded-2xl bg-green-800/60 border-2 border-green-600 p-5 transition-all ${
                    m.link ? "hover:bg-green-800 hover:-translate-y-1 cursor-pointer" : ""
                  }`}
                >
                  <div className="w-11 h-11 rounded-xl bg-amber-300 flex items-center justify-center shadow-md">
                    <m.icon size={19} className="text-green-900" />
                  </div>
                  <p className="mt-3 font-black text-white text-sm uppercase tracking-wide">{m.title}</p>
                  <p className="mt-1 text-xs text-green-100 font-bold break-all">{m.value}</p>
                </Wrapper>
              );
            })}
          </div>
          ...
          {/* Form */}
          <div className="rounded-2xl bg-white p-6 md:p-8 shadow-[8px_8px_0px_0px_rgba(249,115,22,0.9)]">
            ...
            <p className="text-xs text-slate-400 font-black uppercase tracking-wide mt-1">
              You'll get a reply on WhatsApp within minutes
            </p>
            ...
            <div className="mt-5 space-y-4">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-green-700 text-white font-black uppercase tracking-widest text-sm shadow-[4px_4px_0px_0px_rgba(249,115,22,1)] hover:translate-y-0.5 hover:shadow-[2px_2px_0px_0px_rgba(249,115,22,1)] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitted ? (
                  <>
