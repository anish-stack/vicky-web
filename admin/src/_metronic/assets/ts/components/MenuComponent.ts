...
class MenuComponent {
  ...
  public static initGlobalHandlers = () => {
      ...
      if (menuItems && menuItems.length > 0) {
        for (let i = 0; i < menuItems.length; i++) {
          ...
          if (menuObj && menuObj.getItemSubType(item) === 'dropdown') {
            ...
            //-------------forFilterMenu----------------------------------//
            const attributes = menuObj.element.attributes;
            const hasDataKtMenuFilter = Array.from(attributes).some(attr => attr.name === 'data-kt-menu-filter');
            //------------------------------------------------------------//
    ...
    window.addEventListener('resize', () => {
      ...
      throttle(
        ...
        () => {
          // Locate and update Drawer instances on window resize
          const elements = document.querySelectorAll('[data-kt-menu-filter="true"]')
          elements.forEach((el) => {
            const menu = MenuComponent.getInstance(el as HTMLElement)
            if (menu) {
              menu.update()
  ...
  public static bootstrap = () => {
    ...
    MenuComponent.createInstances('[data-kt-menu-filter="true"]')
