...
const validationSchema = Yup.object().shape({
  ...
  discount_cities: Yup.array()
    ...
    .of(
      Yup.object().shape({
        pickup_city_place_id: Yup.string().required("Please select the city"),
    ...
    .test("unique-city-pairs", "Duplicate city pair found", function (discount_cities, context) {
      ...
      const errors = discount_cities.map((item, index) => {
        const key = `${item.pickup_city_place_id}`;
        if (seen.has(key)) {
          return this.createError({
            path: `${context.path}[${index}].pickup_city_place_id`,
...
const Add: FC<Props> = ({ model, isLoading, ModuleName }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getModels(`slug=roundTrip`) as any;
        if (data.status == true) {
  ...
  console.log("discount_cities", formik.values.discount_cities);
                        ...
                        <div key={`discount_cities.${cityIndex}.id`} className='mt-2'>
                          <div className='property-faq position-relative'>
                            ...
                            <div className='row row-cols-2 g-2'>
                              <div className='col'>
                                <label className='fw-semibold fs-6 mb-2 required'>Select City</label>
                            ...
                            <div className='row row-cols-1'>
                              {(!cityObj?.discount_trip_types || cityObj.discount_trip_types?.length == 0) && (
                                <span
                                  ...
                                  onClick={() => {
                                    formik.setFieldValue(`discount_cities.[${cityIndex}].discount_trip_types.[0].trip_type`, 'roundTrip');
