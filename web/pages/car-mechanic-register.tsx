...
export default function CarMechanicRegister() {
  ...
  const [kycStage, setKycStage] = useState<
    "aadhaar-input" | "payment" | "aadhaar-otp" | "done"
  >("aadhaar-input");
  ...
  useEffect(() => {
    try {
      ...
      const savedId =
        sessionStorage.getItem(MECH_ID_KEY) || searchParams.get("mid") || "";
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem(MECH_ID_KEY, mechanicId);
    } catch (_) {}
  ...
  // ── auto search driver once enough digits typed (full number or last 8) ──
  useEffect(() => {
  ...
  const searchDriver = async (phoneNumber: string) => {
    ...
    try {
      ...
      setDriverError(
        err?.response?.data?.message || "Couldn't search right now. Try again.",
      );
    } finally {
...
const changeAadhaarAfterPayment = () => {
    ...
    try {
      ...
      if (form.referralPhone) {
        ...
        if (selectedDriver?._id)
          fd.append("referralDriverId", selectedDriver._id);
        if (selectedDriver?.driver_name)
          fd.append("referralDriverName", selectedDriver.driver_name);
      }
  ...
  // frontend — updated bits only

  // confirmAadhaarNumber: skip payment stage if fee already paid on this record
  const confirmAadhaarNumber = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/${mechanicId}`);
      if (res.data?.data?.isKycFeeDone) {
        await sendAadhaarOtpHandler(); // fee already paid, aadhaar can be changed freely
        return;
      }
    } catch (_) {}
    setKycStage("payment");
  ...
  // startKycPayment: handle alreadyPaid flag from backend
  const startKycPayment = async () => {
    setPayingKyc(true);
    try {
      const orderRes = await axios.post(
        `${API_BASE}/${mechanicId}/kyc/create-order`,
      );
  ...
  const sendAadhaarOtpHandler = async () => {
    ...
    try {
      await axios.post(`${API_BASE}/${mechanicId}/kyc/aadhaar/send-otp`, {
        aadhaarNumber,
      });
  ...
  const verifyAadhaarOtpHandler = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "KYC Verified!",
        text: "Your workshop profile is live.",
        timer: 1800,
        showConfirmButton: false,
      });
    } catch (err: any) {
...
const Header = (
  ...
  if (registered) {
    ...
    const aadhaarMasked = ad?.aadhaarNumber
      ? `XXXX XXXX ${String(ad.aadhaarNumber).slice(-4)}`
      : "";
        ...
        <div className="max-w-md mx-auto px-4 py-8">
          <div className="text-center mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-900">
              Registration Submited Profile Under Review
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              Your workshop is listed. Our team will call you to finish the
            ...
            <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
              ...
              <div className="p-5 space-y-3 text-sm">
                <Row
                  ...
                  value={
                    <span
                      className={`font-semibold ${userData.kycStatus === "kyc-success" ? "text-emerald-600" : "text-amber-500"}`}
                    >
                      {userData.kycStatus === "kyc-success"
                        ? "Verified"
                        : userData.kycStatus}
                    </span>
                  }
                ...
                <Row
                  ...
                  value={
                    <span className="font-semibold text-gray-900">
                      ₹{userData.howMuchItsPaid || 0} paid
                    </span>
                  }
                />
                {aadhaarMasked && (
                ...
                {vd.full_name && (
                  <Row
                    ...
                    label="Aadhaar Name"
                    value={
                      <span className="text-gray-900">{vd.full_name}</span>
                    }
                  />
                )}
                {vd.dob && (
                  <Row
                    ...
                    label="DOB"
                    value={<span className="text-gray-900">{vd.dob}</span>}
                  />
                )}
                {form.referralPhone && (
                  <Row
          ...
          <button
            ...
            className="w-full h-11 mt-5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 transition"
          >
            Done
          </button>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          {STEPS.map((label, i) => (
            ...
            >
              {i !== 0 && (
                <div
                  className={`absolute top-4 right-1/2 w-full h-0.5 -z-10 ${i <= step ? "bg-red-600" : "bg-gray-200"}`}
                />
              )}
              <div
              ...
              <span className="text-[10px] mt-1 text-gray-500 text-center">
                {label}
              </span>
            </div>
          ))}
              ...
              <div className="pt-2">
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-1.5">
                    <FileImage className="w-4 h-4" /> Workshop Photos
                  </label>
                ...
                <p className="text-[11px] text-gray-400 mt-1.5">
                  Up to {MAX_GALLERY} photos. Your profile photo is taken from
                  Aadhaar automatically.
                </p>
              </div>
                    ...
                    {selectedDriver && (
                      <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <span
                            className={`inline-flex items-center gap-1 mt-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                              isDriverKycDone(selectedDriver)
                                ? "bg-emerald-100 text-emerald-700"
                                : "bg-amber-100 text-amber-700"
                            }`}
                          >
              ...
              {kycStage === "aadhaar-input" && (
                <>
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      Your photo, name and address are pulled from Aadhaar
                      automatically
                    </p>
                  </div>
                  <input
                  ...
                  />
                  <button
                    onClick={confirmAadhaarNumber}
                    className="w-full h-12 rounded-xl bg-red-600 text-white font-semibold flex items-center justify-center gap-2"
                  >
              ...
              {kycStage === "payment" && (
                <>
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      One-time fee. OTP is sent to your Aadhaar-linked mobile
                      right after payment.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    disabled={payingKyc || kycFeeLoading}
                    className="w-full h-12 rounded-xl bg-red-600 text-white font-semibold flex items-center justify-center gap-2 disabled:opacity-60"
                  >
                    {payingKyc ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                  ...
                  <button
                    onClick={() => setKycStage("aadhaar-input")}
                    disabled={payingKyc}
