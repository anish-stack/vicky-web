...
export default function TourPackages() {
  ...
  return (
    <section id="tours" className="bg-pink-700 relative">
      <div className="py-12 md:py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 md:mb-14">
          ...
          <p className="mt-3 text-pink-100 font-semibold max-w-xl mx-auto">
            Toll, taxes, fuel and driver — everything included. Only parking extra.
          </p>
        </div>
          ...
          {displayed.map((pkg) => (
            ...
            >
              ...
              <div className="p-3 pb-0">
                <div className="relative overflow-hidden rounded-t-[70px] rounded-b-xl border-2 border-pink-700/30">
                  <img
                    src={pkg.image}
                    alt={pkg.title}
                    className="w-full h-48 md:h-52 object-cover group-hover:scale-105 transition-transform duration-500"
                  />
                  <span className="absolute bottom-3 left-3 inline-flex items-center gap-1.5 bg-white text-pink-800 text-[11px] font-black uppercase tracking-wide px-3 py-1.5 rounded-lg shadow">
                    <Clock size={12} className="text-amber-500" /> {pkg.duration}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 md:p-6 flex flex-col flex-1">
                ...
                <div className="mt-5 pt-4 border-t-2 border-dashed border-pink-700/20 flex items-center justify-between mt-auto">
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Fixed Price</p>
                    <p className="text-xl font-black text-pink-700">{pkg.price}</p>
                  </div>
                  <button
                    onClick={() => enquire(pkg)}
                    className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-pink-700 text-white text-xs font-black uppercase tracking-wide hover:bg-amber-500 transition-colors"
                  >
        ...
        {packages.length > 6 && (
          <div className="text-center mt-10">
            <button
              ...
              className="px-8 py-3 rounded-xl bg-white text-pink-800 text-sm font-black uppercase tracking-wide shadow-[4px_4px_0px_0px_rgba(245,158,11,1)] hover:translate-y-0.5 hover:shadow-[2px_2px_0px_0px_rgba(245,158,11,1)] transition-all"
            >
