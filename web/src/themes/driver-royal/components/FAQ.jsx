      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          ...
          <h2 className="mt-5 text-3xl md:text-5xl font-black text-slate-900">
            Sawal <span className="text-green-700">Aapke, Jawab Hamare</span>
          </h2>
        </div>
            ...
            return (
              <div
                ...
                className={`rounded-2xl bg-white border-2 transition-all ${
                  isOpen
                    ? "border-green-700 shadow-[5px_5px_0px_0px_rgba(249,115,22,0.6)]"
                    : "border-green-700/20 hover:border-green-700/50"
                }`}
              >
                <button
                  onClick={() => setOpen(isOpen ? null : i)}
                  className="w-full flex items-center justify-between gap-4 px-5 py-4.5 py-4 text-left"
                >
                  <span className="font-black text-slate-900 text-sm md:text-base">{f.q}</span>
                  <span
                    className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                      isOpen ? "bg-green-700 text-white" : "bg-green-50 text-green-700"
                    }`}
                  >
