...
export default function Footer() {
  ...
  return (
    <footer className="bg-green-950">
      <Tricolor className="h-2" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <p className="text-center text-[11px] font-black uppercase tracking-[0.35em] text-green-500 mb-5">
          ✦ Guest is God · Pleasant Journey, Honest Rates ✦
        </p>
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-sm font-bold text-green-200/70">
            ...
            {socials.map(({ key, url, Icon }) =>
              url ? (
                <a
                  ...
                  aria-label={key}
                  className="w-8 h-8 rounded-lg bg-green-900 border border-green-800 flex items-center justify-center text-green-300 hover:bg-amber-300 hover:text-green-900 hover:border-amber-300 transition-colors"
                >
                  <Icon size={14} />
                </a>
              ) : null
