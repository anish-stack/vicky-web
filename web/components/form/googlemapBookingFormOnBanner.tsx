...
const GoogleBookingFormOnBannerTwo: React.FC<GoogleBookingFormOnBannerTwoProps> = ({
    Title,
    subTitle,
    buttonName,
}) => {
    ...
    const fetchStopData = async (query: any) => {
        try {
            if (query) {
                const response = await fetch(`${process.env.API_URL}/api/map/autocomplete?query=${query}`);
    ...
    const calculateDistance = async () => {
        try {
            if (allPlaces?.length >= 2) {
                ...
                const totalDistance = data.rows[0].elements.reduce((sum: any, element: any) => {
                    return sum + element?.distance?.value;
                }, 0);
    ...
    }, [startOption, endOption, origins, destinations])
                ...
                <div className='form mt-3'>
                    <div className="trip-options">
                        <div className="d-flex row px-3">
                            ...
                            {distance && (
                                <p className='text-start mt-1 mb-0'>Total Distance: {(distance / 1000).toFixed(2)} km</p>
                            )}
